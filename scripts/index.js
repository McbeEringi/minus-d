...
draw=async({
	...
	msg=false,
	infill=true,
	id=`toko-ess:${Math.floor(Math.random()*0xffffffff).toString(16).padStart(8,0)}`,
	hc,hcl
}={})=>(
	...
	await[...Array(Math.ceil(size.y/chunk.y))].reduce(async(a,y,j,{length:l})=>(
		y=(j==l-1?size.y%chunk.y:chunk.y),
		a=await a,
